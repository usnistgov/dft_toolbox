...
def modify_coordinates(coords, box_size):
    """
    Returns wrapped coordinate values, for the coordinate_wrapper function which modifies the .xyz file. 
...
def coordinate_wrapper(fname, box_size):
    """
    Modifies the .xyz file based on the box size to wrap the coordinates.
    ...
    for num, line in enumerate(lines):
        atom_types.append(line[0])
    x_coords = modify_coordinates(coords[0], box_size)
    y_coords = modify_coordinates(coords[1], box_size)
    z_coords = modify_coordinates(coords[2], box_size)
    with open(fname, 'w') as f:
        f.write("\n")
        for num, line in enumerate(lines):
            toWrite = '\n' + ' '*10 + atom_types[num] + ' '*10 + f'{x_coords[num]:.5f}' + f'     {y_coords[num]:.5f}' + f'     {z_coords[num]:.5f}'
...
def checkOscillatingJob(runningJobIDs):
    """
    Determine if job is oscillating. Two types: sinusoidal type, such as methyl rotation, or other piecewise oscillation, such as what occurs during optimization of large systems with explicit solvent.
...
def checkErrorLogs(username, errorLogs, outputLogs):
    """
    Determine status of G16 job on SLURM. Run this function in a directory in which G16 jobs are running on HPC. 
    ...
    username : str
        Your userID on HPC cluster. 
    errorLogs : list
        A list of strings providing the path to the error logs to be checked.
    outputLogs : list
        A corresponding list of paths to the output logs to be checked. 
    ...
    slurmCommand = 'squeue -u {} --noheader --format="%A"'.format(username)
    process = subprocess.Popen(slurmCommand.split(), stdout=subprocess.PIPE)
    vals = process.communicate()[0].split('\n')
    squeue = [val.rstrip('"').lstrip('"') for val in vals if len(val)>0]
    ...
    for i in range(len(errorLogs)):
        ...
        for line in lines:
            if "error" in line or "Error" in line or "ERROR" in line:
                ...
                if jobIDs[i] not in running:
                    completed.append(jobIDs[i])
    print(running, completed, failed)
...
def create_g16_input(fname, nodes, mem, partition, GasRouteSection, PCMRouteSection, coordFile, charge=0, spinMultiplicity=1):
    """
    Create G16 input and associated .slurm submission script, from either a .xyz coordinates file or a CONVERGED .out/.log G16 optimization file.
...
>>>>>>> 1bce41e689f9e438febd479b92400ef24a1ef20a
